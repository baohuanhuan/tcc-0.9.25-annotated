                           ...
                           Sym **nested_list, Sym *s, struct macro_level **can_read_stream)
{
    ...
    add_cstr1:
        ...
        /* 将当前宏's'加入到nested_list，避免递归展开 */
        sym_push2(nested_list, s->v, 0, 0);
        macro_subst(tok_str, nested_list, mstr, can_read_stream);

        /* pop nested defined symbol */
        /* 将当前宏's'弹出nested_list */
        sa1 = *nested_list;
...
/* 处理'##'操作符，如果没有'##'操作符则返回NULL。否则返回'##'连接后的字符串 */
                ...
                if ((tok >= TOK_IDENT || tok == TOK_PPNUM) && 
                    ...
                    error_pasting:
                        /* NOTE: because get_tok_str use a static buffer,
                           we must save it */
                        /* 注意：因为get_tok_str使用了静态变量，所以我们需要
                            保存它。*/
                        cstr_reset(&cstr);
                        p1 = get_tok_str(tok, &tokc);
                        cstr_cat(&cstr, p1);
...
static void next(void)
{
 ...
 redo:
    if (parse_flags & PARSE_FLAG_SPACES)
        ...
        next_nomacro();
    if (!macro_ptr) {
        /* 如果不是从已替换的宏列表获取的token，则尝试将该token替换 */
